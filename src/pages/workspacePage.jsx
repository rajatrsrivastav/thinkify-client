...
const mockProblemData = {
  "problem-1": {
    ...
    description:
      "Write a function called findMaximum that takes a vector of integers as input and returns the maximum value in the array.",
    requirements: [
...
const Card = ({ children, className }) => (
  <div
    className={`bg-black/20 backdrop-blur-lg border border-white/10 rounded-2xl ${className}`}
  >
    {children}
  </div>
);
...
const Input = (props) => (
  <input
    {...props}
    className={`w-full bg-white/5 p-3 pr-12 rounded-lg outline-none focus:ring-2 focus:ring-teal-500 border border-transparent ${props.className}`}
  />
);
const Button = ({ children, className, ...props }) => (
  <button
    {...props}
    className={`inline-flex items-center justify-center px-4 py-2 rounded-lg font-medium transition-colors ${className}`}
  >
    {children}
  </button>
);
    ...
    <div className="h-screen bg-gray-900 flex text-white relative p-4 gap-4">
      ...
      <div className="relative z-10 w-96 bg-black/20 backdrop-blur-lg border border-white/10 rounded-2xl flex flex-col">
        ...
        <div className="p-4 border-t border-white/10 space-y-3">
          ...
          <Button
            variant="outline"
            className="w-full text-gray-300 border-white/20 bg-transparent hover:bg-white/10"
          >
